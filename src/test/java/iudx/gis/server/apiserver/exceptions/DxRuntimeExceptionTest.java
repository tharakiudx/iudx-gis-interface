package iudx.gis.server.apiserver.exceptions;
...
public class DxRuntimeExceptionTest {
    ...
    public void testConstructor2(VertxTestContext vertxTestContext)
    {
        int statusCode = 400;
        String message= "Checking";
        ResponseUrn responseUrn = ResponseUrn.INVALID_OPERATION;
        DxRuntimeException obj = new DxRuntimeException(statusCode,responseUrn,message);
        assertEquals(400,obj.getStatusCode());
        assertEquals(ResponseUrn.INVALID_OPERATION, obj.getUrn());
        assertEquals("Checking", obj.getMessage());
        vertxTestContext.completeNow();
    }
    @Test
